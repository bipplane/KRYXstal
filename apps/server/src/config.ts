...
const envSchema = z.object({
  ...
  REVIEW_ARTIFACT_MAX_FILES: z.coerce.number().int().min(1).max(1_000).default(50),
  REVIEW_ARTIFACT_MAX_TOTAL_BYTES: z.coerce.number().int().min(1).default(10_485_760),
  REVIEW_ARTIFACT_MAX_RESPONSE_BYTES: z.coerce.number().int().min(1).default(1_048_576),
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    reviewArtifactMaxFiles: env.REVIEW_ARTIFACT_MAX_FILES,
    reviewArtifactMaxTotalBytes: env.REVIEW_ARTIFACT_MAX_TOTAL_BYTES,
    reviewArtifactMaxResponseBytes: env.REVIEW_ARTIFACT_MAX_RESPONSE_BYTES,
